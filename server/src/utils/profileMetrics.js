function calculateProfileCompletion(user, profile) {
  ...
  const checks = [
    user.name || pd.fullName,
    user.bio,
    user.branch || pd.branch,
    user.graduationYear || user.year || pd.year,
  ...
  if (profile.goal === 'Placement & Paid Internship Track') {
    checks.push(profile.interestedDomain);
  }

  const validChecks = checks.filter(Boolean).length;
  return Math.round((validChecks / checks.length) * 100);
}
