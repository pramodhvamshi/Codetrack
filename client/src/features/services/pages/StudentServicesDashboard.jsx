...
export function StudentServicesDashboard() {
  const { token } = useAuth();
    ...
    <AppShell active="services">
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '1.5rem', padding: '1rem 0' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: '1.8rem', fontWeight: 900, color: '#ffffff', letterSpacing: '-0.02em' }}>Student Services Portal</h1>
            <p style={{ margin: '0.3rem 0 0 0', fontSize: '0.85rem', color: '#94a3b8' }}>Request leave permissions, book mentoring sessions, and report laptop hardware issues</p>
          </div>
          ...
          <div style={{ display: 'flex', background: 'rgba(9, 13, 22, 0.9)', padding: '0.4rem', borderRadius: '16px', border: '1px solid rgba(59, 130, 246, 0.25)', gap: '0.4rem', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
            ...
            >
              <Laptop size={16} /> Laptop Requests
            </button>
          </div>
        </div>
        ...
        {/* Tab Content Views */}
        <div>
          {activeTab === 'leave' && <LeaveRequestTab role="student" token={token} />}
          {activeTab === 'mentoring' && <MentoringTab role="student" token={token} />}
          {activeTab === 'laptop' && <LaptopManagementTab role="student" token={token} />}
        </div>
      </div>
    </AppShell>
  );
